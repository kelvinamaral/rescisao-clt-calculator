jQuery(document).ready(function($){
    $("#calcBtn").on("click", function(){
      ...
      if (dismissal < admission) {
        alert(
          "Erro: a data de demissão não pode ser anterior à data de admissão."
        );
        return;
      }

      const diffYears =
        dismissal.getFullYear() -
      ...
      const diffMonths =
        (dismissal.getFullYear() - admission.getFullYear()) * 12 +
      ...
      const feriasVenc = feriasVencidas
        ? (salary / 30) * feriasDias * (4 / 3)
        : 0;

      const totalBruto =
        saldoSalario + avisoValor + decimo + feriasPropComTerco + feriasVenc;
      ...
      $("#result").html(`
            ...
            <ul>
                ...
                <li>Aviso prévio (${avisoDays} dias): R$ ${avisoValor.toFixed(
        2
      )}</li>
                <li>13º proporcional: R$ ${decimo.toFixed(2)}</li>
                <li>Férias proporcionais +1/3: R$ ${feriasPropComTerco.toFixed(
                  2
                )}</li>
                ${
                  feriasVenc > 0
                    ? `<li>Férias vencidas +1/3: R$ ${feriasVenc.toFixed(
                        2
                      )}</li>`
                    : ""
                }
